...
namespace PDBundleModPatcher
{
    ...
    public partial class MainForm : Form
    {
        ...
        /// </summary>
        private const string FullVersion = "1.16.2.7";

        /// <summary>
        ///     The current version.
        /// </summary>
        private const double Version = 1.1627;

        /// <summary>
        ///     The compatible versions.
        /// </summary>
        public double[] Versions = { 1.1627, 1.1626, 1.1625, 1.1623, 1.16, 1.15, 1.14 };
        ...
        private void AddReplacementButtonClick(object sender, EventArgs e)
        {
            ...
            if (fileReplacementType_ComboBox.Text.Equals("Sound Bank Patch")) //a very stupid way to check this. kill me.
            {
        ...
        private void LoadPackageNames()
        {
            ...
            foreach (String pkg in packages)
            {
                ...
                ulong pkgname_hash;
                if (!ulong.TryParse(pkgname, NumberStyles.HexNumber, CultureInfo.InvariantCulture, out pkgname_hash))
                {
                    StaticStorage.log.writeLine("Failed converting package name: " + pkgname);
                }

                    pkgname_hash =
                        ((0x00000000000000FF) & (pkgname_hash >> 56)
                        | (0x000000000000FF00) & (pkgname_hash >> 40)
                        | (0x0000000000FF0000) & (pkgname_hash >> 24)
                        | (0x00000000FF000000) & (pkgname_hash >> 8)
                        | (0x000000FF00000000) & (pkgname_hash << 8)
                        | (0x0000FF0000000000) & (pkgname_hash << 24)
                        | (0x00FF000000000000) & (pkgname_hash << 40)
                        | (0xFF00000000000000) & (pkgname_hash << 56));

                    string packagename = StaticStorage.Known_Index.GetAny(pkgname_hash);
        ...
        private void updateModsListView()
        {
            ...
            foreach (BundleMod localMod in modsList.Values.ToList())
            {
                ...
                if (searchMod(localMod, this.availiableModsSearch_textbox.Text) && (!(localMod.type == BundleMod.ModType.mod_override && !StaticStorage.settings.ShowOverrideMods)))
        ...
        private void refreshModsListView()
        {
            ...
            this.filecontrol_button.Text = (filecontrolDictionary.Count > 0 ? "(" + filecontrolDictionary.Count + ") " : "") + "File Control";
        ...
        public bool TestAssetsFolder()
        {
            ...
            {
                if (File.Exists(Path.Combine(StaticStorage.settings.AssetsFolder, "bundle_db.blb")) && TestBundleForIDStrings("all_14")) //Payday 2
                {
                    StaticStorage.settings.Game = "Payday 2";
                ...
                else if (File.Exists(Path.Combine(StaticStorage.settings.AssetsFolder, "all.blb")) && TestBundleForIDStrings("all_0")) //Payday 2 Demo
                {
                ...
                else if (File.Exists(Path.Combine(StaticStorage.settings.AssetsFolder, "all.blb")) && TestBundleForIDStrings("all_7")) //Payday: The Heist
                {
        ...
        private void savePDMod_Click(object sender, EventArgs e)
        {
            ...
            if (!Directory.Exists(Path.GetTempPath() + "mod\\"))
                Directory.CreateDirectory(Path.GetTempPath() + "mod\\");
            ...
            using (FileStream fs = new FileStream(Path.Combine(Path.GetTempPath(), Path.GetFileName(saveDialog.FileName)), FileMode.CreateNew))
            {
                using (StreamWriter sw = new StreamWriter(fs))
                {
                    ...
                    foreach (var item in this.newMod.ItemQueue)
                    {
                        sw.WriteLine(item.SourceFile + " : " + "mod\\" + Path.GetFileName(item.ReplacementFile));
                    }
                }
            }
        ...
        private void hashButton_Click(object sender, EventArgs e)
        {
            ...
            if (this.hashUseHexcheckBox.Checked)
            {
                ...
                this.hashHashBox.Text = string.Format("0x{0:X}", hash);
            }
            else
            ...
            try
            {
                ...
                if (this.hashUseHexcheckBox.Checked)
                {
                    ...
                    if (!ulong.TryParse(hashVal, NumberStyles.HexNumber, CultureInfo.InvariantCulture, out hash))
                        MessageBox.Show(
                    "Error while converting from hex to ulong",
                    "Could not reverse the hash");
                ...
                {
                    this.hashTextBox.Text = StaticStorage.Known_Index.GetAny(hash);
                }
                else
                {
                    MessageBox.Show(
                    "Entered hash is not part of this game.",
                    "Could not reverse the hash");
                }
            }
            catch (Exception ex)
        ...
        public void Load_settings()
        {
            ...
            this.RenameTexture.Checked = StaticStorage.settings.ReplaceTexture;
            this.ExtractFolderText.Text = StaticStorage.settings.CustomExtractPath;
            
            this.ExtractAllBox.Checked = StaticStorage.settings.ExtractAll;
            this.ListBox1.Checked = StaticStorage.settings.List;
            this.XMLExtensions.Checked = StaticStorage.settings.XMLExtensions;
            this.BikExtensions.Checked = StaticStorage.settings.BIKExtensions;
            this.WriteConsole.Checked = StaticStorage.settings.WriteConsole;
            this.ListData.Checked = StaticStorage.settings.ListData;
            this.ListLogBox.Text = StaticStorage.settings.ListLogFile;
            StaticStorage.extract.extract_one = false;
            StaticStorage.extract.extract_id = "";

            if (this.SingleBundleCheck.Checked)
            {
                this.SingleBundleText.Enabled = true;
                //this.ExtractAllBox.Enabled = false;
                StaticStorage.extract.extract_one = true;
            }
            else
            {
                //if (!this.ListBox1.Checked)
                //{
                   // this.ExtractAllBox.Enabled = true;
                //}
            ...
            {
                this.ExtractAllBox.Enabled = true;
                this.ListData.Enabled = true;
            }

            if (this.ListData.Checked)
            {
            ...
            //list_only = StaticStorage.settings.List;
            
            //extract_all = StaticStorage.settings.ExtractAll;
            
            
        }

        void TextWriteLine(string line, object line2 = null, object line3 = null)
        {
            //this.ExtractLogBox.Text += StaticStorage.log.writeLine(string.Format(line, line2, line3), true);
            this.ExtractLogBox.AppendText(StaticStorage.log.writeLine(string.Format(line, line2, line3), true));
            //this.ExtractLogBox.SelectionStart = this.ExtractLogBox.Text.Length;
            this.ExtractLogBox.ScrollToCaret();
        }


        public Thread bundleThread;
        System.Windows.Forms.Timer ExtractTimer;
        ...
        private void ReplaceTextureChanged(object sender, EventArgs e)
        {
            StaticStorage.settings.ReplaceTexture = this.RenameTexture.Checked;
            this.SaveSettings();
        }

        private void ExtractSingleBundleChanged(object sender, EventArgs e)
        {
            if (this.SingleBundleCheck.Checked)
            {
                //this.ExtractAllBox.Enabled = false;
            ...
            StaticStorage.extract.extract_one = this.SingleBundleCheck.Checked;
        }

        private void ListCheckedChanged(object sender, EventArgs e)
        {
            StaticStorage.settings.List = this.ListBox1.Checked;
            //list_only = this.ListBox1.Checked;
            if (this.ListBox1.Checked)
            {
                this.ExtractAllBox.Enabled = false;
                this.ListData.Enabled = false;
        ...
        private void ExtractAllCheckedChanged(object sender, EventArgs e)
        {
            StaticStorage.settings.ExtractAll = this.ExtractAllBox.Checked;
            //extract_all = this.ExtractAllBox.Checked;
            this.SaveSettings();
        }

        private void SingleBundleTextChanged(object sender, EventArgs e)
        {
            StaticStorage.extract.extract_id = this.SingleBundleText.Text;
        }

        private void XMLExtensionsChanged(object sender, EventArgs e)
        {
            StaticStorage.settings.XMLExtensions = this.XMLExtensions.Checked;
            this.SaveSettings();
        }

        private void BikExtensionsCheckedChanged(object sender, EventArgs e)
        {
            StaticStorage.settings.BIKExtensions = this.BikExtensions.Checked;
            this.SaveSettings();
        }
        ...
        private void FinishExtract()
        {
            ...
            logtracker = 0;
            this.StartExtracting.Text = StartLabel;
            this.progressBar1.Value = (int)StaticStorage.extract.progress;
            this.ProgressLabel.Text = "Progress: " + StaticStorage.extract.current_bundle + "/" + StaticStorage.extract.total_bundle;
            //this.ProgressLabel.Text = "Progress: 0/0";
            //this.progressBar1.Value = 0;
            this.ExtractAllBox.Enabled = true;
            this.SingleBundleCheck.Enabled = true;
            this.ListBox1.Enabled = true;
            this.RenameTexture.Enabled = true;
            this.XMLExtensions.Enabled = true;
            this.BikExtensions.Enabled = true;
            StaticStorage.extract.log.Clear();
            bundleThread.Abort();
            bundleThread = null;
            this.ListData.Enabled = true;
            this.ListLogBox.Enabled = true;
            if (this.SingleBundleCheck.Checked)
            {
                //this.ExtractAllBox.Enabled = false;
                this.SingleBundleText.Enabled = true;
            ...
            this.ExtractFolderButton.Enabled = true;
        ...
        {
            LogEntry[] logs = StaticStorage.extract.getLog();
            this.progressBar1.Value = (int)StaticStorage.extract.progress;
            this.ProgressLabel.Text = "Progress: " + StaticStorage.extract.current_bundle + "/" + StaticStorage.extract.total_bundle;
            ...
            if (logs.Length > logtracker)
            {
                ...
                {
                    this.ExtractLogBox.SelectionStart = this.ExtractLogBox.TextLength;
                    this.ExtractLogBox.SelectionLength = 0;
                    this.ExtractLogBox.AppendText(logs[logtracker].ToString());
                    this.Enabled = StaticStorage.extract.MainEnabled;
                }
        ...
        private void ListDataCheckedChange(object sender, EventArgs e)
        {
            StaticStorage.settings.ListData = this.ListData.Checked;
            this.SaveSettings();
            if (this.ListData.Checked)
            {
                this.ListBox1.Enabled = false;
        ...
        private void BrowseForLogClick(object sender, EventArgs e)
        {
            ...
            this.ListLogBox.Text = openDialog.FileName;
    ...
    public static class StaticStorage
    {
        ...
        /// </summary>
        public static BundleExtraction extract = new BundleExtraction();
